...
def get_file_type(**kwargs):
    ...
    if s3_location.endswith('.csv'):
        ...
        kwargs['ti'].xcom_push(key='partitionColumn',value='Year')
        kwargs['ti'].xcom_push(key='outputBucket',value='s3://midterm-output/movies')
    else:
...
def stop_airflow_ec2():
    ...
    if len(emr_list) == 0:
        ...
        ec2.stop_instances(InstanceIds=['i-0e1bec1f1a73319e9'])
    else:
        print("Add retry timer in future")

stop_af_ec2 = PythonOperator(
...
parse_request.set_upstream(source_file_type)
...
stop_af_ec2.set_upstream(terminate_emr_cluster)